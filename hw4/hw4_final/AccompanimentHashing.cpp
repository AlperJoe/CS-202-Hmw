...
void AccompanimentHashing::removeAccompaniment(string accompanimentName)
{
    int index = hashFunction(accompanimentName);
    int i = 0;
    do
    {
        index = (index + i * i) % tableSize;
        i++;
    } while (hashTable[index] != nullptr && hashTable[index]->accompanimentName != accompanimentName);

    Accompaniment* temp = hashTable[index];
    delete temp;
